...
exports.createItem = async (req, res) => {
    ...
    try {
        ...
        const itemResult = await db.query(
            'INSERT INTO items (name, description, price, item_type, image_url, seller_id, college_id) VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING *',
            [name, description, price, item_type, image_url, seller_id, college_id]
        );
        res.status(201).json({ msg: 'Item created successfully', item: itemResult.rows[0] });
    } catch (error) {
        console.error(error);
        res.status(500).send('Server error');
    }
    ...
    try {
        ...
        const { rows } = await db.query(
            `SELECT items.*, users.name AS seller_name 
             FROM items 
             JOIN users ON items.seller_id = users.id 
             WHERE items.status = 'available' AND items.name ILIKE $1`, // ILIKE is for case-insensitive search in PostgreSQL
            [searchTerm]
        );
        res.json(rows);
    } catch (error) {
