...
const MyItemsPage = () => {
    ...
    const fetchMyItems = async () => {
        ...
        try {
            const res = await axios.get('https://student-sharing-api.onrender.com/api/items/user/my-items', config);
            setMyItems(res.data);
        } catch (error) {
    ...
    const handleDelete = async (itemId) => {
        if (window.confirm('Are you sure you want to delete this item?')) {
            const config = { headers: { 'x-auth-token': token } };
            try {
                await axios.delete(`https://student-sharing-api.onrender.com/api/items/${itemId}`, config);
                fetchMyItems(); 
            } catch (error) {
